...
export default async function ProtocoloDetalhePage({
  ...
  const { data: tiposAnexo } = await supabase.from("tipos_anexo").select("id, nome").eq("ativo", true);

  const { data: anexos } = await supabase
    .from("anexos")
    .select("id, tramitacao_id, nome_arquivo, caminho_storage, tipos_anexo:tipo_anexo_id(nome)")
    .eq("protocolo_id", protocolo.id);
  ...
  const anexosComUrl = await Promise.all(
    (anexos ?? []).map(async (a: any) => {
      const { data: signed } = await supabase.storage
        ...
        .createSignedUrl(a.caminho_storage, 60 * 60);
          ...
          {despachos?.map((d: any, idx: number) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {anexosComUrl
                  .filter((a: any) => a.tramitacao_id === d.id)
                  .map((a: any) => (
                    <a
                      key={a.id}
                      href={a.url ?? "#"}
                    ...
                    >
                      📎 {a.nome_arquivo}
                      {a.tipos_anexo?.nome && (
                        <span style={{ color: "var(--ink-faint)" }}>— {a.tipos_anexo.nome}</span>
                      )}
                    </a>
                  ))}
              </div>
